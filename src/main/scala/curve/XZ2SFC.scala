...
import curve.XZ2SFC.{QueryWindow, XElement}
...
class XZ2SFC(g: Short, xBounds: (Double, Double), yBounds: (Double, Double)) {
  ...
  def getLength(xmin: Double, ymin: Double, xmax: Double, ymax: Double, lenient: Boolean = false): Int = {
    val (nxmin, nymin, nxmax, nymax) = normalize(xmin, ymin, xmax, ymax, lenient)
   ...
   * @return
   */
  private def ranges(query: Array[XZ2SFC.QueryWindow], rangeStop: Int): Seq[IndexRange] = {
    ...
    result.append(current)
    //println("xz2:" + level + "_" + ranges.size() + "_" + result.size)
    result
  }
   ...
   * @return
   */
  def sequenceCode(x: Double, y: Double, length: Int): Long = {
    var xmin = 0.0
    var ymin = 0.0
    var xmax = 1.0
    var ymax = 1.0
